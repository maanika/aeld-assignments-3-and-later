...
#define CMD_IOCSEEKTO "AESDCHAR_IOCSEEKTO:"
...
static void *handle_connection(void *args)
{
	...
	while ((bytes = recv(client_fd, buffer + bytes_read, MAX_RECV_SIZE, 0)) > 0) {
		...
		if (end != NULL) {
			...
			strncpy(str, buffer,
				str_len - 1); // ignore new line when printing and leave it for NULL
			syslog(LOG_INFO, "received message: %s\n", str);

			if (strncmp(str, CMD_IOCSEEKTO, strlen(CMD_IOCSEEKTO)) == 0) {
				uint32_t cmd;
				uint32_t cmd_offset;
				if (sscanf(str, CMD_IOCSEEKTO "%d,%d", &cmd, &cmd_offset) != 2) {
					syslog(LOG_ERR, "Failed to parse seekto command");
					break;
				}

				struct aesd_seekto seekto = {.write_cmd = cmd,
							     .write_cmd_offset = cmd_offset};

				pthread_mutex_lock(&mutex);
				*ret = ioctl(fd, AESDCHAR_IOCSEEKTO, &seekto);
				...
				// Ensure the read of the file and return over the socket uses the
				// same (not closed and re-opened) file descriptor used to send the
				// ioct
				pthread_mutex_lock(&mutex);
				char buffer[BUFFER_SIZE] = {0};
				int bytes_read = 0;
				while ((bytes_read = read(fd, buffer, sizeof(buffer))) > 0) {
					...
					if (bytes_sent != bytes_read) {
						syslog(LOG_ERR, "send_client: bytes read from file "
	...
	while (!exit_requested) {
		...
		SLIST_FOREACH_SAFE(datap, &head, entries, tmp)
		{
			if (datap->thread_args->flag) {
				if (pthread_join(datap->thread_args->thread_id, &return_value) ==
				    0) {
					free(return_value);
				}
